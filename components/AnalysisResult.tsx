...
const Card = ({ title, icon: Icon, content, score, colorClass, extraBadges, fallback = FALLBACK_ANALYSIS_TEXT }: any) => {
...
const AnalysisResult: React.FC<AnalysisResultProps> = ({ analysis }) => {
  const normalizedAnalysis = normalizeText(analysis);
  const pillars = normalizedAnalysis.bazi || [];
    ...
    <div className="w-full space-y-8 animate-fade-in-up">
      ...
      <div className="bg-gradient-to-br from-indigo-50 to-white p-6 rounded-xl border border-indigo-100 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
          ...
          <div className="w-full md:w-1/3">
            <ScoreBar score={normalizedAnalysis.summaryScore} />
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <Card
          title="币圈交易运势"
          icon={Bitcoin}
          content={normalizedAnalysis.crypto}
          score={normalizedAnalysis.cryptoScore}
          colorClass="text-amber-600"
          extraBadges={
            <>
              <span className="px-2 py-1 bg-amber-50 text-amber-700 text-xs font-bold rounded border border-amber-200">
                暴富流年: {normalizedAnalysis.cryptoYear}
              </span>
              <span className="px-2 py-1 bg-indigo-50 text-indigo-700 text-xs font-bold rounded border border-indigo-200">
                推荐: {normalizedAnalysis.cryptoStyle}
              </span>
            </>
          }
        />

        <Card title="性格分析" icon={Brain} content={normalizedAnalysis.personality} score={normalizedAnalysis.personalityScore} colorClass="text-teal-600" />
        <Card title="事业行业" icon={Briefcase} content={normalizedAnalysis.industry} score={normalizedAnalysis.industryScore} colorClass="text-blue-600" />
        <Card title="发展风水" icon={Compass} content={normalizedAnalysis.fengShui} score={normalizedAnalysis.fengShuiScore} colorClass="text-cyan-700" />
        <Card title="财富层级" icon={Coins} content={normalizedAnalysis.wealth} score={normalizedAnalysis.wealthScore} colorClass="text-amber-600" />
        <Card title="婚姻情感" icon={Heart} content={normalizedAnalysis.marriage} score={normalizedAnalysis.marriageScore} colorClass="text-pink-600" />
        <Card title="身体健康" icon={Activity} content={normalizedAnalysis.health} score={normalizedAnalysis.healthScore} colorClass="text-emerald-600" />
        <Card title="六亲关系" icon={Users} content={normalizedAnalysis.family} score={normalizedAnalysis.familyScore} colorClass="text-purple-600" />
          ...
          content={
            <div className="space-y-4">
              ...
              <p className="text-xs text-black leading-relaxed border-t border-gray-100 pt-2 text-justify">
                注：命运还受环境和个人选择影响，八字趋势不能完全代表真实人生。命理学不是玄学，而是帮助我们在人生列车上做出更好选择的哲学工具。
              </p>
            </div>
          }
        />
